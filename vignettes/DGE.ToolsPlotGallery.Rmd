...
```{r obsPlot2, echo=TRUE, warning=FALSE, message=FALSE, fig.width = 7, fig.height = 5}
  ...
  myplots <- obsPlot2(tidyInt, plotByCol="GeneID",
                      ...
                      facet = FALSE)
  #plot the first one
  printAndSave(myplots[[1]], "obsPlot1.png")
...
```{r logRatioPlot, echo=TRUE, warning=FALSE, message=FALSE, fig.width = 7, fig.height = 5}
  ...
  printAndSave(myLogRatioPlot, "logRatioPlot.png")
```
...
```{r checkSex, echo=TRUE, warning=FALSE, message=FALSE, fig.width = 7, fig.height = 5}
  ...
  printAndSave(sexPlot, "logRatioPlot.png")
...
#dispersion plot
dispersionPlot <- plotDisp(dgelist, designMatrix, lineFit="loess", plotType="BCV")
printAndSave(dispersionPlot, "dispersionPlot.png")
#BCV (biological coefficient of variation) plot
BCV_Plot <- plotDisp(dgelist, designMatrix, lineFit="loess", plotType="BCV")
printAndSave(BCV_Plot, "BCV_Plot.png")
...
printAndSave(boxNorm, "boxNorm.png")

densityNorm <- plotNorm(dgeObj, plotType="density", normalize="tmm")
printAndSave(densityNorm, "densityNorm.png")
...
This function is intended to work on Omicsoft-style alignment QC data.  QC parameters are named 
in the first column and subsequent columns hold data for each sample.  

Since Omicsoft alignment QC includes hundreds of metrics, you typically want to limit the plots
to a subset of the available metrics.
...
```{r QCplots, echo=TRUE, warning=FALSE, message=FALSE, fig.width = 7, fig.height = 5 }
  ...
  #pick a column from the design table with shorter sample names
  colnames(qcdat) <- c("Metric", make.names(dgeObj$design$Sample.Name))
  #
  
  # qcdat <- qcdat[complete.cases(qcdat),]
  
   for (i in 2:ncol(qcdat)){ print(class(qcdat[,i,drop=TRUE]))}
  ...
  MyQCplots <- QCplots(qcdat, metricNames=someFavMetrics, debug=TRUE) #all defaults
...
You need to download a QC data file from the S3 bucket bms-ngs-arrayserver or, preferrably, you can read data
directly from S3 using software to mount an S3 bucket to a local location.

To map and S3 bucket to local storage:  
