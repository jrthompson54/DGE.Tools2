...
#Get raw count data and annotation from an Omicsoft project
...
#Get raw count data and annotation from an Xpress project
...
Use function Xpress2DGEO from the 
[Xpress2R package](https://biogit.pri.bms.com/thompj27/Xpress2R)
to retrieve the counts and annotation.

```{r RetrieveFromXpress, eval=FALSE}
library(Xpress2R)
dgeObj20135 = Xpress2DGEO(20135)
```
...
```{r DispersionPlot, eval=FALSE, echo=TRUE, warning=FALSE, message=FALSE}
...
If SVA found 1 or more variables, it created a new design matrix and you need
to re-run runVoom with the new design matrix.  We'll now show how to re-run runVoom with the new designMatrix.
...
Simple print:  
```{r simplePrint, results='asis'}
library(knitr)
print(dgeObj)
```

Verbose print:  
```{r verbosePrint, results='asis'}
