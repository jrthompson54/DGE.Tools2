...
take care to preserve the rownames in th design and gene annotation dataframes.  
...
in the design table is more acurately call "pretreatment" as it defines which
...
```{r DispersionPlot, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
...
If SVA find surrogate variables,  you cbind the surrogate variables to the design table create a new 
formula including the SVA variables as terms then run voom/lmFit as you would normally.   
...
If SVA found 1 or more variables, it created a new design matrix with the SV
columns added and you can include the SV columns in your formula.

If there are more than a few SV found, we suggest you use the variantPartition
package to evaluate the variance fraction associated with each SV and consider
using just the top few SVs in your formula.  To accomplish this, you need to
manually edit the design matrix and remove SV columns you don't want to use.
...
```{r runVoom, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r postvoom_snapshot, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
# Let's save a snapshot of the DGEobj at this point
saveRDS (dgeObj, file.path(outputPath, "postvoom_dgeobj.RDS"))
```
...
```{r pvalueHistograms, echo=TRUE, warning=FALSE, message=FALSE}
...
[The training slides are available here](https://biogit.pri.bms.com/thompj27/DGE.Tools2/blob/master/DGE_Tools_Training_Mar2019.pptx)  
...
Install the latest stable versions of the DGEobj, DGE.Tools2 and JRTutil packages from BRAN using install.packages.  
